...
function MatchInner() {
  ...
  const load = useCallback(async () => {
    ...
    const [m, { data: rulesRows }] = await Promise.all([
    ...
    if (m?.allConsented) {
      ...
      setContacts(
        Object.fromEntries(((data ?? []) as RevealedContact[]).map((c) => [c.profile_id, c]))
      );
    } else {
      setContacts({});
    }
  ...
  if (!match) {
    return (
      <div className="space-y-4">
        ...
        <p className="card text-sm text-sand-500">This match no longer exists.</p>
      </div>
    );
  }
  ...
  return (
    <div className="card">
      ...
      <div className="mt-3 flex flex-wrap gap-1">
        {showSeniority && years != null && <span className="badge bg-sand-100 text-sand-600">~{years} yrs service</span>}
