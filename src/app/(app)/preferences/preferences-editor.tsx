  ...
  return (
    <div className="space-y-5">
      <div className="card space-y-3">
        <h2 className="font-semibold text-slate-900">Add a preferred location</h2>
      ...
      <div className="card">
        <h2 className="mb-3 font-semibold text-slate-900">Your preferences (highest priority first)</h2>
        {rows.length === 0 ? (
          <p className="text-sm text-slate-500">No preferences yet. Add at least one above.</p>
        ) : (
          <ol className="space-y-2">
            {rows.map((r, i) => (
              <li key={`${r.preferred_state}-${r.preferred_district}`} className="flex items-center justify-between rounded-lg border border-slate-200 px-3 py-2">
